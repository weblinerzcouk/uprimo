        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-md-12 add-new-btn">
                    <a type="button" class="btn btn-success pull-right" href="{{url('/admin/add_attribute')}}" data-toggle="add_new" >
                        + Add New
                    </a>
                </div>
            </div>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="zero_config" class="table table-striped border white-box">
                                    ...
                                    <tbody id="termsrow">
                                    @foreach($attribute as $attribute_row)
                                    <tr id="row{{$attribute_row->id}}">
                                        <td>{{$attribute_row->id}}</td>
                                        <td>{{$attribute_row->name}}</td>
                                        <td>
                                            @if($attribute_row->status==1)
                                        ...
                                        <td>
                                            <a class="btn btn-success" href="{{url('/admin/edit_attribute/'.$attribute_row->id)}}" data-toggle="edit_attribute">
                                                <span class="fa fa-pencil"></span>
                                            </a>
                                            <a class="btn btn-danger" href="{{url('/admin/delete_attribute/'.$attribute_row->id)}}" data-toggle="delete_attribute">
...
<script>
    ...
    $(document).on('click','a[data-toggle=edit_attribute]',function (e) {
        e.preventDefault();
        $('#edit_modal').find('.modal-body').load($(this).attr('href'));
        $('#edit_modal').modal('show');
    });
    //delete_country
    $(document).on('click','a[data-toggle=delete_attribute]',function (e) {
        e.preventDefault();
        var str = '';
        $.ajax({
            url         : $(this).attr('href'),
